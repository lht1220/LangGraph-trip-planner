...
def _build_react_agent(model, tools: List[BaseTool], system_prompt: str):
    """创建 ReAct 智能体，兼容不同 langgraph 版本的系统提示参数名。

    langgraph >= 0.2 使用 ``prompt`` 参数；更早版本使用 ``state_modifier``。
    """
    try:
...
def create_attraction_search_agent(tools: List[BaseTool]):
    ...
    try:
        ...
        # 创建智能体图
        agent_graph = _build_react_agent(llm, tools, system_prompt)

        logger.info(f"[OK] 景点搜索智能体创建成功，工具数量: {len(tools)}")
        return agent_graph
...
def create_weather_agent(tools: List[BaseTool]):
    ...
    try:
        ...
        logger.info(f"[OK] 天气查询智能体创建成功，工具数量: {len(tools)}")
        return agent_graph
