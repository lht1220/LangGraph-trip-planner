...
class TripRequest(BaseModel):
    """旅行规划请求"""
    city: str = Field(..., description="目的地城市", example="北京")#...代表字段必须提供，没有默认值
...
# Optional[str]也就是说，这个字段的值可以是字符串，也可以是 None。
...
class WeatherInfo(BaseModel):
    ...
    #Pydantic v2 的字段验证器
    @field_validator('day_temp', 'night_temp', mode='before')#mode='before'在类型验证之前执行（非常关键）
    @classmethod
