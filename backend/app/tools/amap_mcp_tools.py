...
def run_async(coro):
    ...
    在 FastAPI/uvicorn 的请求上下文中调用时，外层已经有一个运行中的
    事件循环，此时 `asyncio.run()` 无法再创建并运行一个新循环。
    本函数检测这种情况，把协程交给独立线程（拥有自己的事件循环）执行；
    若当前没有运行中的循环，则直接 `asyncio.run()`。
    """
    try:
        ...
        if (has_arun and not has_run) or (is_structured_tool and has_arun):
            ...
            class SyncWrapper(tool.__class__):
                def _run(self, *args, **kwargs):
                    ...
                    通过模块级 run_async() 执行：当处于运行中的事件循环时，
                    它会自动切换到一个独立线程的新事件循环，避免
                    asyncio.run() 在运行中循环内报错（典型场景：uvicorn 请求上下文）。
                    """
...
def get_amap_mcp_tools() -> List[BaseTool]:
    """同步获取MCP工具"""
    try:
        # run_async 已通过独立线程处理"已有事件循环中运行"的情况，无需 nest_asyncio
        return run_async(create_amap_mcp_tools())
    except Exception as e:
...
def get_amap_essential_tools() -> List[BaseTool]:
    ...
    try:
        ...
        tools = run_async(load_and_filter())
