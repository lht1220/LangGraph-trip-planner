...
"""
    ...
    def _safe_int(self, value: Any, default: int = 0) -> int:
        ...
        if isinstance(value, str):
            v = value.strip()
            if not v or v in ("免费", "无", "None", "null", "-"):
                return default
            try:
                return int(float(v))
                ...
                for item in data:
                    if isinstance(item, dict):
                        ...
                        attraction = Attraction(
                            ...
                            visit_duration=self._safe_int(item.get("visit_duration"), 120),
                            description=item.get("description") or "",
                            category=item.get("category") or "景点",
                            ticket_price=self._safe_int(item.get("ticket_price"), 0)
                        )
    ...
    def _parse_trip_plan(self, response: str, request: TripRequest) -> TripPlan:
        ...
        try:
            json_str = self._extract_json(response)
            # 容错：LLM 可能在 JSON 后追加说明文字，用 raw_decode 只解析第一个完整 JSON 对象
